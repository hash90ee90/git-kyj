...
function add(a, b) { // 함수 정의 시 소괄호 안에 정의된 키워드는 파라미터
  console.log(a + b);

}

// 함수 호출
add (3, 4); // 3, 4는 인자(argument)
...
function log(a) { //한 개의 파라미터 선언
  console.log(a);
}

log(); //아규먼트 X (파라미터의 개수보다 적음) - undefined
log('안녕'); // 한 개의 아규먼트 (파라미터의 개수와 동일) 
log('Hello', `hi`); // 두개의 아규먼트(파라미터의 개수보다 많음) - 지정된 수의 파라미터 이상의 아규먼트는 읽히지 X
