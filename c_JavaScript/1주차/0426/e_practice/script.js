...
// 컴퓨터는 사용자가 숫자를 맞출 때까지 숫자를 입력받고
// , 사용자가 입력한 숫자가 정답보다 높은지, 낮은지, 아니면 정확한지 출력

// 게임은 사용자가 숫자를 맞췄을 때 종료

// 사용자가 입력한 숫자가 유효한 숫자인지 확인하고, 그렇지 않으면 오류 메시지를 출력하고 다른 숫자를 재입력받기

//? 문제 상세 설명
// 프로그램은 먼저 컴퓨터에 의해 1부터 100 사이의 무작위 숫자가 생성
// Math.random(): 0과 1사이의 무자위 수를 생성
...
while (attempts > 0) {
  const userInput = prompt(`1부터 100사이의 숫자를 추측해주세요. 남은 횟수는 ${attempts}번 입니다.`);
  
  // 사용자가 입력한 값을 정수로 변환
  //  Numver() 형 변화는 정수와 실수 모두를 숫자형으로 변환
  const guess = parseInt(userInput);

  // boolean의 false값을 나타내는 데이터
  // : 0 , ' ', NaN, null, undefinded
  if (!guess || guess < 1 || guess > 100 ) {
    console.log(`유효한 숫자를 입력해주세요.`);
    // 현재 반복을 즉시 중단, 루프의 다음 반복을 재시작
    //  continue  이후의 코드가 실행 X, 루프의 조건식으로 돌아감
    continue;
  }
  //입력한 숫자가 유효한 경우
