...
switch (식) {
  ...
  ...
  default:
    식이 어떤 값과도 일치하지 않을 때 실행되는 코드(필수사항 X)
}
*/
...
switch (fruit) {
  // : 조건 값과 일치한 case가 존재하면 그 이후의 코드를 모두 실행
  case 'apple':
    console.log(`사과입니다.`);
    break;
  case 'banana' :
  ...
  case `orange` :
    console.log(`오렌지 입니다.`);
    break;
    // : 이전의 선택이 표현식의 값과 일치한다면, 브라우저는 해당 코드 블럭에서 실행을 멈추고, switch문 아래에 있는 코드로 이동
...
score = 65;
// or 연산자 사용 - 두 조건을 동시에 부합하는 값이 존재하지 X
let grade =
