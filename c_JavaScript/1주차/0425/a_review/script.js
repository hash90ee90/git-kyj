...
console.log(`현재 ${city}의 기온은 ${temperature}도이며, 비가 오고 있습니다.`  `현재 ${city}의 기온은 ${temperature}도이며, 비가 오지 않습니다. `);
...
let originalPrice = Number(prompt(`원가를 입력하세요.`)); // 원가 입력받음
...
// 플레이어의 점수가 5000점 이상이어야 하고,
// 플레이어가 특별 트로피를 가지고 있거나, 또는 플레이어가 3개 이상의 게임에서 우승해야 함.

// 단락 평가를 사용하여, 만약 플레이어의 점수가 이미 5000점 이상이면, 추가 조건은 평가하지 않도록 스크립트를 구성

let score = Number(prompt(`점수를 입력해 주세요.`)) ;
let hasTrophy = prompt(`트로피를 가지고 있습니까? yes/no `) === `yes`; // 사용자로부터 트로피를 가지고 있는지 답변을 받아 변수에 저장 (yes/no의 데이터 입력받기 - yes와 동일할 경우 해당 변수에 true값 저장)
let gameWins = Number(prompt(`우승한 게임의 수를 입력하세요.`));
