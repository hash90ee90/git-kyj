...
//  2) 결합 방식: 수식 내에 우선순위가 같은 연산자가 둘 이상 있을 떄, 먼저 어느 연산을 수행할 것인가를 결정
// >> 각각의 연산자 종류에 따라 상이
...
x = 10 ;
console.log(x++); //후위 증가 연산자: +1이 해당 값에서 바로 적용되지 않음
console.log(x++);
console.log(x);
...
x = 10;
console.log(--x); //전위 감소 연산자: -1이 해당 값에서 바로 적용
console.log(--x);
console.log(++x);
...
console.log(num1 != num2); //부등  fasle
console.log(num1 === num2); //일치 fasle
